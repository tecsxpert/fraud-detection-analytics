...
public class TransactionServiceImpl implements TransactionService {
    ...
    // ✅ FIX: use primitive long (no null issues)
    private long getCurrentUserId() {
        return 1L;
    }

    // 🔒 Reusable ownership validation (IDOR protection)
    private void validateOwnership(Transaction transaction, Long id) {
        long currentUserId = getCurrentUserId();

        if (transaction.getCreatedBy() == null || !transaction.getCreatedBy().equals(currentUserId)) {
            log.warn("Unauthorized access attempt for transaction id: {}", id);
    ...
    public Optional<Transaction> getTransactionById(Long id) {
        ...
        // 🔒 IDOR Protection
        validateOwnership(transaction, id);
